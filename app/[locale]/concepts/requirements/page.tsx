...
const labels: Record<string, Record<string, string>> = {
  ...
  requirements: { en: 'Requirements', es: 'Requisitos', fr: 'Exigences' },
}
...
export default async function RequirementsPage({
  ...
  const file = await getMarkdownFile(locale, 'concepts/requirements')
  if (!file) notFound()
    ...
    <div className="space-y-6">
      <Breadcrumb
        items={[
          ...
          { title: t('requirements'), href: '/concepts/requirements' },
        ]}
      />
      ...
      <ArticleNavigation currentSlug="requirements" />
    </div>
  )
}
