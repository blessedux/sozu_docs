...
const labels: Record<string, Record<string, string>> = {
  ...
  reference: { en: 'Reference', es: 'Referencia', fr: 'Référence' },
}
...
export default async function ReferencePage({
  ...
  const file = await getMarkdownFile(locale, 'concepts/reference')
  if (!file) notFound()
    ...
    <div className="space-y-6">
      <Breadcrumb
        items={[
          ...
          { title: t('reference'), href: '/concepts/reference' },
        ]}
      />
      ...
      <ArticleNavigation currentSlug="reference" />
    </div>
  )
}
