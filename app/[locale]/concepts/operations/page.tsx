...
const labels: Record<string, Record<string, string>> = {
  ...
  operations: { en: 'Operations', es: 'Operaciones', fr: 'Opérations' },
}
...
export default async function OperationsPage({
  ...
  const file = await getMarkdownFile(locale, 'concepts/operations')
  if (!file) notFound()
    ...
    <div className="space-y-6">
      <Breadcrumb
        items={[
          ...
          { title: t('operations'), href: '/concepts/operations' },
        ]}
      />
      ...
      <ArticleNavigation currentSlug="operations" />
    </div>
  )
}
