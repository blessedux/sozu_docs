...
const labels: Record<string, Record<string, string>> = {
  ...
  architecture: { en: 'Architecture', es: 'Arquitectura', fr: 'Architecture' },
}
...
export default async function ArchitecturePage({
  ...
  const file = await getMarkdownFile(locale, 'concepts/architecture')
  if (!file) notFound()
    ...
    <div className="space-y-6">
      <Breadcrumb
        items={[
          ...
          { title: t('architecture'), href: '/concepts/architecture' },
        ]}
      />
      ...
      <ArticleNavigation currentSlug="architecture" />
    </div>
  )
}
